...
void solve() {
    int n; cin>>n;
    int total_sum = (n * 1LL * (n + 1)) / 2;
    if(total_sum & 1) cout<<"NO\n";
    else {
        ...
        while(s1_sum < half_sum){
            int remaining_sum = half_sum - s1_sum;
            if(max_element < remaining_sum){
                s1.insert(max_element);
                s1_sum += max_element;
                vis[max_element] = 1;
                max_element--;
            } else {
                s1.insert(remaining_sum);
